...
* **Development Server:** `npm run dev` (Runs locally directly from root on `http://localhost:3000`)
...
### 1. Workspace Flattening & Clean Root Setup
* Moved all project directories (`app`, `components`, `lib`, `public`, `supabase`, `agent`, `node_modules`) and configuration files (`package.json`, `.env.local`, `.gitignore`, `tsconfig.json`) directly into the root workspace directory `Mini-ERP-master (2)`.
* Deleted the redundant nested `Mini-ERP-master` inner subfolder.
* `npm run dev` and all CLI commands now execute directly from root.

### 2. Dynamic Admin Sidebar Navigation
* **Centralized Configuration:** Created [`lib/config/navigation.ts`](file:///c:/Users/Acer/Downloads/Mini-ERP-master%20(2)/lib/config/navigation.ts) defining structured navigation sections (`Main Menu`, `Operations Modules`, `Administration`).
* **Live Counter Badges:** Implemented [`lib/hooks/use-sidebar-metrics.ts`](file:///c:/Users/Acer/Downloads/Mini-ERP-master%20(2)/lib/hooks/use-sidebar-metrics.ts) to query real-time counts from Supabase (e.g. low stock alerts, pending orders, registered staff).
* **Sidebar Sync:** Synchronized expanded [`MainSidebar`](file:///c:/Users/Acer/Downloads/Mini-ERP-master%20(2)/components/main-sidebar.tsx) and slim [`SidebarRail`](file:///c:/Users/Acer/Downloads/Mini-ERP-master%20(2)/components/sidebar-rail.tsx) with accurate sub-route active matching.

### 3. Full Removal of Hardcoded Data
...
### 4. Transformation to Furniture & Interior Living Enterprise
...
* Updated Landing Page ([`app/page.tsx`](file:///c:/Users/Acer/Downloads/Mini-ERP-master%20(2)/app/page.tsx)), inventory forms, sales studio client placeholders, and showroom settings.

### 5. Idempotent DDL Database Schema ([`supabase/schema.sql`](file:///c:/Users/Acer/Downloads/Mini-ERP-master%20(2)/supabase/schema.sql))
...
### 6. Input Usability & Stepper Removal
* **Global CSS Fix:** Hidden native browser number input spinners in [`app/globals.css`](file:///c:/Users/Acer/Downloads/Mini-ERP-master%20(2)/app/globals.css).
...
### 7. Furniture Product Edit Feature
* Added **Edit** button (`Edit2` icon) to every product row in [`app/admin/inventory/page.tsx`](file:///c:/Users/Acer/Downloads/Mini-ERP-master%20(2)/app/admin/inventory/page.tsx).
...
### 8. Auto-Calculated Order Amounts & Custom Dropdowns
* **Auto-Calculation:** The *Create Client Order* modal in [`app/admin/sales/page.tsx`](file:///c:/Users/Acer/Downloads/Mini-ERP-master%20(2)/app/admin/sales/page.tsx) automatically pre-fills and computes `Total Amount = Unit Price × Quantity`.
